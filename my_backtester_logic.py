...
def load_and_prepare_data_from_path(
    csv_file_path, num_days_to_process, randomize_period_bool
):
    ...
    if randomize_period_bool:
        ...
        if (max_ts_in_data - min_ts_in_data) < window_duration:
            ...
            possible_start_timestamps = df_full_history.index[
                df_full_history.index <= latest_possible_start_ts
            ].unique()
            if not possible_start_timestamps.empty:
                random_start_ts = random.choice(possible_start_timestamps)
                end_ts_for_slice = min(
                    random_start_ts + window_duration, max_ts_in_data
                )
                df_selected_period = df_full_history.loc[
                    random_start_ts:end_ts_for_slice
                ].copy()
                logger.info(
                ...
                logger.warning(
                    "Could not find valid random start period in CSV. Using latest."
                )
                start_date_for_slice = max(
                    min_ts_in_data, max_ts_in_data - window_duration
                )
                df_selected_period = df_full_history.loc[start_date_for_slice:].copy()
    else:
        start_date_for_slice = max(
            df_full_history.index.min(), df_full_history.index.max() - window_duration
        )
        df_selected_period = df_full_history.loc[start_date_for_slice:].copy()
        logger.info(
    ...
    df_selected_period["sma_30"] = (
        df_selected_period["close"].rolling(window=30, min_periods=1).mean()
    )
    df_selected_period["sma_60"] = (
        df_selected_period["close"].rolling(window=60, min_periods=1).mean()
    )
    df_selected_period["volatility"] = (
        df_selected_period["close"].rolling(window=30, min_periods=1).std()
    )
    df_selected_period["rsi"] = calculate_rsi(df_selected_period["close"])
    df_selected_period["atr"] = calculate_atr(
        df_selected_period["high"],
        df_selected_period["low"],
...
# --- AI Interaction ---
def get_gpt_action_for_web(
    sub_df,
    current_balance,
    current_btc_holdings,
    trade_amount_btc_val,
    current_entry_price,
    ...
    if not openai_api_key_param:
        logger.error(
            "OpenAI API key is NOT SET before API call in get_gpt_action_for_web!"
        )
        return (
            "HOLD",
            0.0,
            "OpenAI API key missing in function call",
            user_strategy_prompt_str,
        )

    client = OpenAI(api_key=openai_api_key_param)
    logger.debug(
        f"Using OpenAI Key: {openai_api_key_param[:5]}...{openai_api_key_param[-4:]} for API call."
    )
    ...
    prompt_to_send = prompt_to_send.replace(
        "{{CURRENT_BTC_HOLDINGS}}", f"{current_btc_holdings:.8f}"
    )
    prompt_to_send = prompt_to_send.replace(
        "{{CURRENT_USD_BALANCE}}", f"{current_balance:.2f}"
    )
    prompt_to_send = prompt_to_send.replace(
        "{{TRADE_AMOUNT_BTC}}", f"{trade_amount_btc_val:.8f}"
    )
    prompt_to_send = prompt_to_send.replace(
        "{{ENTRY_PRICE}}", f"{current_entry_price:.2f}"
    )
    ...
    logger.debug(
        f"Populated prompt for GPT (first 500 chars):\n{prompt_to_send[:500]}..."
    )
    ...
    # ... (rest of error handling same as before) ...
        ...
        if "Rate limit" in str(e):
            logger.warning("Rate limit possibly hit.")
    except Exception as e:
        action, confidence = "HOLD", 0.0
        logger.error(f"Generic error in get_gpt_action: {e}", exc_info=True)
        reasoning = f"Generic LLM Error: {e}"
        if LOG_RAW_GPT_RESPONSE_ON_ERROR_MODULE and content_from_llm:
            logger.error(f"Problematic content: {content_from_llm}")
...
def execute_backtest_strategy(
    ...
    trade_amount_btc=0.01,
    transaction_fee_percent=0.075,
    min_confidence_threshold=60,
    use_python_managed_sltp_safety_net=True,
    ...
    if not openai_api_key_param:
        return {"error": "OpenAI API Key was not provided to backtester."}

    df_full = load_and_prepare_data_from_path(
        csv_data_path_param, num_days_param, randomize_period_from_csv_param
    )
    ...
    total_steps = (
        max(
            1,
            int(
                (simulation_end_time - simulation_start_time).total_seconds()
                / decision_window.total_seconds()
            ),
        )
        if decision_window.total_seconds() > 0
        else 1
    )
    logger.info(
    ...
    with tqdm(
        total=total_steps,
        desc=f"Job {job_id_param}",
        unit="step",
        disable=not (__name__ == "__main__"),
    ) as pbar:
        while current_time <= simulation_end_time:
            ...
            current_atr = (
                current_market_data_df["atr"].iloc[-1]
                if pd.notna(current_market_data_df["atr"].iloc[-1])
                else 0.01
            )
            ...
            if (
                active_position
                and use_python_managed_sltp_safety_net
                and current_atr > 0
            ):
            ...
            if not system_exit_triggered:
                llm_action, llm_confidence, llm_reasoning, populated_prompt = (
                    get_gpt_action_for_web(
                        current_market_data_df,
                        balance_usd,
                        btc_holdings,
                        trade_amount_btc,
                        entry_price if active_position else 0.0,
                        user_strategy_prompt_str,
                        openai_api_key_param,
                        api_call_buffer_seconds,
                        gpt_model,
                    )
                )
                ...
                if active_position:
                    if (
                        llm_action == "SELL"
                        and llm_confidence >= min_confidence_threshold
                    ):
                        ...
                        if llm_action == "SELL":
                            ...
                            final_reasoning_for_step = (
                                "LLM BUY (in pos), holding. LLM: " + llm_reasoning
                            )
                else:
                    if (
                        llm_action == "BUY"
                        ...
                        if llm_action == "BUY":
                            ...
                            final_reasoning_for_step = (
                                "LLM SELL (no pos), holding. LLM: " + llm_reasoning
                            )
            ...
            if final_action_for_step == "BUY" and not active_position:
                ...
                if balance_usd >= cost_pre_fee + fee:
                    ...
                    log_display = "HOLD (Buy Fail Bal)"
                    final_reasoning_for_step += " (Insuff. bal)"
                ...
                if final_action_for_step != "HOLD":
                    log_display = f"HOLD ({final_action_for_step} Fail)"
                if (
                    final_reasoning_for_step == "Default Hold / No signal"
                    and llm_reasoning
                ):
                    final_reasoning_for_step = llm_reasoning
            ...
            if system_exit_triggered:
                action_log_message = f"[SYSTEM {final_action_for_step}] {current_time}: {final_reasoning_for_step}"
            ...
            if (
                ...
                and current_time > simulation_end_time + timedelta(hours=1)
            ):
                logger.error(f"[{job_id_param}] Safety break loop.")
                break

    log_df_output = pd.DataFrame(
        trade_logs_list,
        columns=[
            "timestamp",
            "action",
    ...
    equity_df_output = pd.DataFrame(
        portfolio_history, columns=["timestamp", "equity_usd"]
    )
    if not equity_df_output.empty:
        equity_df_output.set_index("timestamp", inplace=True)
    ...
    try:
        if not equity_df_output.empty:
            ...
            equity_df_output["equity_usd"].plot(
                ax=ax,
                title=f"Equity Curve - Job {job_id_param}",
    ...
    final_equity = (
        equity_df_output["equity_usd"].iloc[-1]
        if not equity_df_output.empty
        else start_balance
    )
    net_pl = final_equity - start_balance
    num_trades = (
        len(log_df_output[log_df_output["action"] == "BUY"])
        if not log_df_output.empty
        else 0
    )
...
if __name__ == "__main__":
    ...
    if not test_openai_key:
        logger.error(
            "OPENAI_API_KEY not found in .env for standalone test. Please create a .env file with your key."
        )
        ...
        logger.warning(
            f"Using placeholder key: {test_openai_key}. OpenAI calls will fail."
        )
    else:
        logger.info(
            f"Using OpenAI key from .env for standalone test: {test_openai_key[:5]}...{test_openai_key[-4:]}"
        )

    dummy_job_id = "standalone_test_003"
    dummy_csv_path = (
        "dummy_btc_data_standalone.csv"  # Ensure this file exists or is created
    )

    # Create a more substantial dummy CSV for better testing of indicators
    timestamps = pd.date_range(
        start="2023-01-01 00:00:00", periods=300, freq="min"
    )  # 5 hours of data
    data = {
...
"""
    ...
    if "error" not in results and results.get(
        "results"
    ):  # Check 'results' sub-dictionary
        logger.info(
            f"Equity curve should be at: {results.get('equity_curve_url')}"
        )  # Access from top level
        logger.info(
            f"Trade log should be at: {results.get('trade_log_url')}"
        )  # Access from top level
