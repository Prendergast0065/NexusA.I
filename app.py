...
logging.basicConfig(
    level=logging.INFO,
    format="[%(asctime)ss] [%(levelname)s] [FlaskAPP] %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
...
def leaderboard_page():
    top_users = User.query.filter(User.total_backtests > 0).order_by(User.total_net_pl.desc()).limit(10).all()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            backtest_results_data = execute_backtest_strategy(
                ...
                job_id_param=job_id,
                output_base_dir=RESULTS_FOLDER,
            ...
            db.session.commit()
            app.logger.info(f"User {current_user.email} stats updated for leaderboard.")
...
def serve_job_result_file(job_id, filename):
    directory = os.path.join(app.root_path, RESULTS_FOLDER, job_id)
        ...
        while True:
            ...
            if random.random() < 0.2:
                ...
                if not User.query.filter_by(email=new_user_email).first():
                    ...
                    db.session.add(new_user)
                    app.logger.info(f"Adding new demo user to leaderboard: {new_user_email}")
            
            db.session.commit()
            app.logger.info("Leaderboard update simulation complete. Sleeping for a while...")
            time.sleep(60)
        ...
        with redirect_stderr(f):
            try:
                ...
                if "Can't locate revision identified by" in f.getvalue():
                    app.logger.warning("Migration history mismatch detected. Stamping database to head.")
                    # Run stamp head if the specific migration error is detected
                    from flask_migrate import stamp
                    stamp()
                    app.logger.info("Database migration history stamped to head. Re-running upgrade.")
                    upgrade()
                else:
